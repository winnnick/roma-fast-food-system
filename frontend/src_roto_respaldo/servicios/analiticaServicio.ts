...
function obtenerClaveSesionAuditoria(
...
function formatearDuracionSesion(
  milisegundos: number,
): string {
...
function construirDuracionesSesiones(
  ...
  const iniciosPorUsuario =
    new Map<
      string,
      RegistroAuditoria[]
    >();

  const eventosAutenticacion =
    [...registros]
      .filter(
        (evento) =>
          evento.modulo ===
  ...
  for (
    const evento
    of eventosAutenticacion
  ) {
    const clave =
      obtenerClaveSesionAuditoria(
        evento,
      );

    const inicios =
      iniciosPorUsuario.get(clave) ??
      [];
...
function construirActividadOperativa(
  ...
  const duracionesSesiones =
    construirDuracionesSesiones(
      datos.auditoria,
    );
